...
let rec decode_trip_replacement_period d =
  ...
  while !continue__ do
    ...
    | Some (2, Pbrt.Bytes) -> begin
      v.replacement_period <- Some (Gtfs_realtime_pb.decode_time_range (Pbrt.Decoder.nested d));
    end
    | Some (2, pk) -> 
...
let rec decode_nyct_feed_header d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.nyct_subway_version <- Pbrt.Decoder.string d; nyct_subway_version_is_set := true;
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(nyct_feed_header), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.trip_replacement_period <- (decode_trip_replacement_period (Pbrt.Decoder.nested d)) :: v.trip_replacement_period;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(nyct_feed_header), field(2)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  begin if not !nyct_subway_version_is_set then Pbrt.Decoder.missing_field "nyct_subway_version" end;
  ({
...
let rec decode_nyct_trip_descriptor d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.train_id <- Some (Pbrt.Decoder.string d);
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(nyct_trip_descriptor), field(1)" pk
    | Some (2, Pbrt.Varint) -> begin
      v.is_assigned <- Some (Pbrt.Decoder.bool d);
    end
    | Some (2, pk) -> 
...
let rec decode_nyct_stop_time_update d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.scheduled_track <- Some (Pbrt.Decoder.string d);
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(nyct_stop_time_update), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.actual_track <- Some (Pbrt.Decoder.string d);
    end
    | Some (2, pk) -> 
...
let rec encode_trip_replacement_period (v:Nyct_subway_types.trip_replacement_period) encoder = 
  begin match v.Nyct_subway_types.route_id with
  | Some x -> 
    Pbrt.Encoder.key (1, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();
  end;
  begin match v.Nyct_subway_types.replacement_period with
  | Some x -> 
...
let rec encode_nyct_trip_descriptor_direction (v:Nyct_subway_types.nyct_trip_descriptor_direction) encoder =
  match v with
...
let rec encode_nyct_trip_descriptor (v:Nyct_subway_types.nyct_trip_descriptor) encoder = 
  begin match v.Nyct_subway_types.train_id with
  | Some x -> 
    Pbrt.Encoder.key (1, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();
  end;
  begin match v.Nyct_subway_types.is_assigned with
  | Some x -> 
    Pbrt.Encoder.key (2, Pbrt.Varint) encoder; 
    Pbrt.Encoder.bool x encoder;
  | None -> ();
  end;
  begin match v.Nyct_subway_types.direction with
  | Some x -> 
...
let rec encode_nyct_stop_time_update (v:Nyct_subway_types.nyct_stop_time_update) encoder = 
  begin match v.Nyct_subway_types.scheduled_track with
  | Some x -> 
    Pbrt.Encoder.key (1, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();
  end;
  begin match v.Nyct_subway_types.actual_track with
  | Some x -> 
    Pbrt.Encoder.key (2, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();

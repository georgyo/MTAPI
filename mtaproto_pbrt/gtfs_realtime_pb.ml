...
type trip_descriptor_mutable =
  ...
  ; mutable schedule_relationship :
      Gtfs_realtime_types.trip_descriptor_schedule_relationship option
  }
...
let default_vehicle_descriptor_mutable () : vehicle_descriptor_mutable =
  { id = None; label = None; license_plate = None }
;;
...
type trip_update_stop_time_update_mutable =
  ...
  ; mutable schedule_relationship :
      Gtfs_realtime_types.trip_update_stop_time_update_schedule_relationship option
  }
...
let default_trip_update_stop_time_update_mutable () : trip_update_stop_time_update_mutable
  ...
  ; schedule_relationship =
      Some
        (Gtfs_realtime_types.default_trip_update_stop_time_update_schedule_relationship
           ())
  }
;;
...
let default_position_mutable () : position_mutable =
  { latitude = 0.; longitude = 0.; bearing = None; odometer = None; speed = None }
;;
...
type vehicle_position_mutable =
  ...
  ; mutable stop_id : string option
  ; mutable current_status :
      Gtfs_realtime_types.vehicle_position_vehicle_stop_status option
  ; mutable timestamp : int64 option
  ; mutable congestion_level :
      Gtfs_realtime_types.vehicle_position_congestion_level option
  ; mutable occupancy_status :
      Gtfs_realtime_types.vehicle_position_occupancy_status option
  }
...
let default_vehicle_position_mutable () : vehicle_position_mutable =
  ...
  ; stop_id = None
  ; current_status =
      Some (Gtfs_realtime_types.default_vehicle_position_vehicle_stop_status ())
  ; timestamp = None
...
let default_time_range_mutable () : time_range_mutable = { start = None; end_ = None }
...
let default_entity_selector_mutable () : entity_selector_mutable =
  { agency_id = None; route_id = None; route_type = None; trip = None; stop_id = None }
;;
...
let default_translated_string_translation_mutable ()
  : translated_string_translation_mutable
  =
  { text = ""; language = None }
;;
...
let default_feed_entity_mutable () : feed_entity_mutable =
  { id = ""; is_deleted = Some false; trip_update = None; vehicle = None; alert = None }
;;
...
let default_feed_message_mutable () : feed_message_mutable =
  { header = Gtfs_realtime_types.default_feed_header (); entity = [] }
;;

let rec decode_feed_header_incrementality d =
  match Pbrt.Decoder.int_as_varint d with
  | 0 ->
    (Gtfs_realtime_types.Full_dataset : Gtfs_realtime_types.feed_header_incrementality)
  | 1 ->
    (Gtfs_realtime_types.Differential : Gtfs_realtime_types.feed_header_incrementality)
...
let rec decode_feed_header d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.gtfs_realtime_version <- Pbrt.Decoder.string d;
      gtfs_realtime_version_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_header), field(1)" pk
    | Some (2, Pbrt.Varint) ->
      v.incrementality <- Some (decode_feed_header_incrementality d)
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_header), field(2)" pk
    | Some (3, Pbrt.Varint) -> v.timestamp <- Some (Pbrt.Decoder.int64_as_varint d)
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_header), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !gtfs_realtime_version_is_set
  then Pbrt.Decoder.missing_field "gtfs_realtime_version";
...
let rec decode_trip_descriptor_schedule_relationship d =
  match Pbrt.Decoder.int_as_varint d with
  | 0 ->
    (Gtfs_realtime_types.Scheduled
      : Gtfs_realtime_types.trip_descriptor_schedule_relationship)
  | 1 ->
    (Gtfs_realtime_types.Added
      : Gtfs_realtime_types.trip_descriptor_schedule_relationship)
  | 2 ->
    (Gtfs_realtime_types.Unscheduled
      : Gtfs_realtime_types.trip_descriptor_schedule_relationship)
  | 3 ->
    (Gtfs_realtime_types.Canceled
      : Gtfs_realtime_types.trip_descriptor_schedule_relationship)
...
let rec decode_trip_descriptor d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Bytes) -> v.trip_id <- Some (Pbrt.Decoder.string d)
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(trip_descriptor), field(1)" pk
    | Some (5, Pbrt.Bytes) -> v.route_id <- Some (Pbrt.Decoder.string d)
    | Some (5, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(trip_descriptor), field(5)" pk
    | Some (6, Pbrt.Varint) -> v.direction_id <- Some (Pbrt.Decoder.int32_as_varint d)
    | Some (6, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(trip_descriptor), field(6)" pk
    | Some (2, Pbrt.Bytes) -> v.start_time <- Some (Pbrt.Decoder.string d)
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(trip_descriptor), field(2)" pk
    | Some (3, Pbrt.Bytes) -> v.start_date <- Some (Pbrt.Decoder.string d)
    | Some (3, pk) ->
...
let rec decode_vehicle_descriptor d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Bytes) -> v.id <- Some (Pbrt.Decoder.string d)
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(vehicle_descriptor), field(1)" pk
    | Some (2, Pbrt.Bytes) -> v.label <- Some (Pbrt.Decoder.string d)
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(vehicle_descriptor), field(2)" pk
    | Some (3, Pbrt.Bytes) -> v.license_plate <- Some (Pbrt.Decoder.string d)
    | Some (3, pk) ->
...
let rec decode_trip_update_stop_time_event d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Varint) -> v.delay <- Some (Pbrt.Decoder.int32_as_varint d)
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(trip_update_stop_time_event), field(1)" pk
    | Some (2, Pbrt.Varint) -> v.time <- Some (Pbrt.Decoder.int64_as_varint d)
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(trip_update_stop_time_event), field(2)" pk
    | Some (3, Pbrt.Varint) -> v.uncertainty <- Some (Pbrt.Decoder.int32_as_varint d)
    | Some (3, pk) ->
...
let rec decode_trip_update_stop_time_update_schedule_relationship d =
  ...
  | 1 ->
    (Gtfs_realtime_types.Skipped
      : Gtfs_realtime_types.trip_update_stop_time_update_schedule_relationship)
  | 2 ->
    (Gtfs_realtime_types.No_data
      : Gtfs_realtime_types.trip_update_stop_time_update_schedule_relationship)
  | _ ->
    Pbrt.Decoder.malformed_variant "trip_update_stop_time_update_schedule_relationship"
;;
...
let rec decode_trip_update_stop_time_update d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Varint) -> v.stop_sequence <- Some (Pbrt.Decoder.int32_as_varint d)
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(trip_update_stop_time_update), field(1)" pk
    | Some (4, Pbrt.Bytes) -> v.stop_id <- Some (Pbrt.Decoder.string d)
    | Some (4, pk) ->
    ...
    | Some (5, Pbrt.Varint) ->
      v.schedule_relationship
        <- Some (decode_trip_update_stop_time_update_schedule_relationship d)
    | Some (5, pk) ->
...
let rec decode_trip_update d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.trip <- decode_trip_descriptor (Pbrt.Decoder.nested d);
      trip_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(trip_update), field(1)" pk
    | Some (3, Pbrt.Bytes) ->
      v.vehicle <- Some (decode_vehicle_descriptor (Pbrt.Decoder.nested d))
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(trip_update), field(3)" pk
    | Some (2, Pbrt.Bytes) ->
      v.stop_time_update
        <- decode_trip_update_stop_time_update (Pbrt.Decoder.nested d)
           :: v.stop_time_update
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(trip_update), field(2)" pk
    | Some (4, Pbrt.Varint) -> v.timestamp <- Some (Pbrt.Decoder.int64_as_varint d)
    | Some (4, pk) -> Pbrt.Decoder.unexpected_payload "Message(trip_update), field(4)" pk
    | Some (5, Pbrt.Varint) -> v.delay <- Some (Pbrt.Decoder.int32_as_varint d)
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(trip_update), field(5)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !trip_is_set then Pbrt.Decoder.missing_field "trip";
...
let rec decode_position d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bits32) ->
      v.latitude <- Pbrt.Decoder.float_as_bits32 d;
      latitude_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(position), field(1)" pk
    | Some (2, Pbrt.Bits32) ->
      v.longitude <- Pbrt.Decoder.float_as_bits32 d;
      longitude_is_set := true
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(position), field(2)" pk
    | Some (3, Pbrt.Bits32) -> v.bearing <- Some (Pbrt.Decoder.float_as_bits32 d)
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(position), field(3)" pk
    | Some (4, Pbrt.Bits64) -> v.odometer <- Some (Pbrt.Decoder.float_as_bits64 d)
    | Some (4, pk) -> Pbrt.Decoder.unexpected_payload "Message(position), field(4)" pk
    | Some (5, Pbrt.Bits32) -> v.speed <- Some (Pbrt.Decoder.float_as_bits32 d)
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(position), field(5)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !longitude_is_set then Pbrt.Decoder.missing_field "longitude";
  if not !latitude_is_set then Pbrt.Decoder.missing_field "latitude";
...
let rec decode_vehicle_position_vehicle_stop_status d =
  match Pbrt.Decoder.int_as_varint d with
  | 0 ->
    (Gtfs_realtime_types.Incoming_at
      : Gtfs_realtime_types.vehicle_position_vehicle_stop_status)
  | 1 ->
    (Gtfs_realtime_types.Stopped_at
      : Gtfs_realtime_types.vehicle_position_vehicle_stop_status)
  | 2 ->
    (Gtfs_realtime_types.In_transit_to
      : Gtfs_realtime_types.vehicle_position_vehicle_stop_status)
...
let rec decode_vehicle_position_congestion_level d =
  ...
  | 1 ->
    (Gtfs_realtime_types.Running_smoothly
      : Gtfs_realtime_types.vehicle_position_congestion_level)
  | 2 ->
    (Gtfs_realtime_types.Stop_and_go
      : Gtfs_realtime_types.vehicle_position_congestion_level)
  | 3 ->
    (Gtfs_realtime_types.Congestion
      : Gtfs_realtime_types.vehicle_position_congestion_level)
  | 4 ->
    (Gtfs_realtime_types.Severe_congestion
      : Gtfs_realtime_types.vehicle_position_congestion_level)
...
let rec decode_vehicle_position_occupancy_status d =
  match Pbrt.Decoder.int_as_varint d with
  | 0 ->
    (Gtfs_realtime_types.Empty : Gtfs_realtime_types.vehicle_position_occupancy_status)
  | 1 ->
    (Gtfs_realtime_types.Many_seats_available
      : Gtfs_realtime_types.vehicle_position_occupancy_status)
  | 2 ->
    (Gtfs_realtime_types.Few_seats_available
      : Gtfs_realtime_types.vehicle_position_occupancy_status)
  | 3 ->
    (Gtfs_realtime_types.Standing_room_only
      : Gtfs_realtime_types.vehicle_position_occupancy_status)
  | 4 ->
    (Gtfs_realtime_types.Crushed_standing_room_only
      : Gtfs_realtime_types.vehicle_position_occupancy_status)
  | 5 ->
    (Gtfs_realtime_types.Full : Gtfs_realtime_types.vehicle_position_occupancy_status)
  | 6 ->
...
let rec decode_vehicle_position d =
  ...
  while !continue__ do
    ...
    | Some (2, Pbrt.Bytes) -> v.position <- Some (decode_position (Pbrt.Decoder.nested d))
    | Some (2, pk) ->
    ...
    | Some (7, Pbrt.Bytes) -> v.stop_id <- Some (Pbrt.Decoder.string d)
    | Some (7, pk) ->
    ...
    | Some (5, Pbrt.Varint) -> v.timestamp <- Some (Pbrt.Decoder.int64_as_varint d)
    | Some (5, pk) ->
...
let rec decode_time_range d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Varint) -> v.start <- Some (Pbrt.Decoder.int64_as_varint d)
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(time_range), field(1)" pk
    | Some (2, Pbrt.Varint) -> v.end_ <- Some (Pbrt.Decoder.int64_as_varint d)
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(time_range), field(2)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({ Gtfs_realtime_types.start = v.start; Gtfs_realtime_types.end_ = v.end_ }
...
let rec decode_entity_selector d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Bytes) -> v.agency_id <- Some (Pbrt.Decoder.string d)
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(entity_selector), field(1)" pk
    | Some (2, Pbrt.Bytes) -> v.route_id <- Some (Pbrt.Decoder.string d)
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(entity_selector), field(2)" pk
    | Some (3, Pbrt.Varint) -> v.route_type <- Some (Pbrt.Decoder.int32_as_varint d)
    | Some (3, pk) ->
    ...
    | Some (5, Pbrt.Bytes) -> v.stop_id <- Some (Pbrt.Decoder.string d)
    | Some (5, pk) ->
...
let rec decode_translated_string_translation d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.text <- Pbrt.Decoder.string d;
      text_is_set := true
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload
        "Message(translated_string_translation), field(1)"
        pk
    | Some (2, Pbrt.Bytes) -> v.language <- Some (Pbrt.Decoder.string d)
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload
        "Message(translated_string_translation), field(2)"
        pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !text_is_set then Pbrt.Decoder.missing_field "text";
  ({ Gtfs_realtime_types.text = v.text; Gtfs_realtime_types.language = v.language }
...
let rec decode_translated_string d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) ->
      v.translation
        <- decode_translated_string_translation (Pbrt.Decoder.nested d) :: v.translation
    | Some (1, pk) ->
...
let rec decode_alert d =
  ...
  while !continue__ do
    ...
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(alert), field(1)" pk
    | Some (5, Pbrt.Bytes) ->
      v.informed_entity
        <- decode_entity_selector (Pbrt.Decoder.nested d) :: v.informed_entity
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(alert), field(5)" pk
    | Some (6, Pbrt.Varint) -> v.cause <- Some (decode_alert_cause d)
    | Some (6, pk) -> Pbrt.Decoder.unexpected_payload "Message(alert), field(6)" pk
    | Some (7, Pbrt.Varint) -> v.effect <- Some (decode_alert_effect d)
    | Some (7, pk) -> Pbrt.Decoder.unexpected_payload "Message(alert), field(7)" pk
    | Some (8, Pbrt.Bytes) ->
      v.url <- Some (decode_translated_string (Pbrt.Decoder.nested d))
    | Some (8, pk) -> Pbrt.Decoder.unexpected_payload "Message(alert), field(8)" pk
    | Some (10, Pbrt.Bytes) ->
      v.header_text <- Some (decode_translated_string (Pbrt.Decoder.nested d))
    | Some (10, pk) -> Pbrt.Decoder.unexpected_payload "Message(alert), field(10)" pk
    | Some (11, Pbrt.Bytes) ->
      v.description_text <- Some (decode_translated_string (Pbrt.Decoder.nested d))
    | Some (11, pk) -> Pbrt.Decoder.unexpected_payload "Message(alert), field(11)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
...
let rec decode_feed_entity d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) ->
      ...
      id_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_entity), field(1)" pk
    | Some (2, Pbrt.Varint) -> v.is_deleted <- Some (Pbrt.Decoder.bool d)
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_entity), field(2)" pk
    | Some (3, Pbrt.Bytes) ->
      v.trip_update <- Some (decode_trip_update (Pbrt.Decoder.nested d))
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_entity), field(3)" pk
    | Some (4, Pbrt.Bytes) ->
      v.vehicle <- Some (decode_vehicle_position (Pbrt.Decoder.nested d))
    | Some (4, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_entity), field(4)" pk
    | Some (5, Pbrt.Bytes) -> v.alert <- Some (decode_alert (Pbrt.Decoder.nested d))
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_entity), field(5)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !id_is_set then Pbrt.Decoder.missing_field "id";
  ({ Gtfs_realtime_types.id = v.id
...
let rec decode_feed_message d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.header <- decode_feed_header (Pbrt.Decoder.nested d);
      header_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_message), field(1)" pk
    | Some (2, Pbrt.Bytes) ->
      v.entity <- decode_feed_entity (Pbrt.Decoder.nested d) :: v.entity
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(feed_message), field(2)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !header_is_set then Pbrt.Decoder.missing_field "header";
  ({ Gtfs_realtime_types.header = v.header; Gtfs_realtime_types.entity = v.entity }
    : Gtfs_realtime_types.feed_message)
;;

let rec encode_feed_header_incrementality
...
let rec encode_feed_header (v : Gtfs_realtime_types.feed_header) encoder =
  ...
  (match v.Gtfs_realtime_types.incrementality with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Varint) encoder;
     encode_feed_header_incrementality x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.timestamp with
   | Some x ->
...
let rec encode_trip_descriptor_schedule_relationship
...
let rec encode_trip_descriptor (v : Gtfs_realtime_types.trip_descriptor) encoder =
  (match v.Gtfs_realtime_types.trip_id with
   | Some x ->
     Pbrt.Encoder.key (1, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.route_id with
   | Some x ->
     Pbrt.Encoder.key (5, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.direction_id with
   | Some x ->
     Pbrt.Encoder.key (6, Pbrt.Varint) encoder;
     Pbrt.Encoder.int32_as_varint x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.start_time with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.start_date with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.schedule_relationship with
   | Some x ->
...
let rec encode_vehicle_descriptor (v : Gtfs_realtime_types.vehicle_descriptor) encoder =
  (match v.Gtfs_realtime_types.id with
   | Some x ->
     Pbrt.Encoder.key (1, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.label with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.license_plate with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
...
let rec encode_trip_update_stop_time_event
  (v : Gtfs_realtime_types.trip_update_stop_time_event)
  encoder
  =
  (match v.Gtfs_realtime_types.delay with
   | Some x ->
     Pbrt.Encoder.key (1, Pbrt.Varint) encoder;
     Pbrt.Encoder.int32_as_varint x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.time with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Varint) encoder;
     Pbrt.Encoder.int64_as_varint x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.uncertainty with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Varint) encoder;
     Pbrt.Encoder.int32_as_varint x encoder
   | None -> ());
...
let rec encode_trip_update_stop_time_update_schedule_relationship
...
let rec encode_trip_update_stop_time_update
  (v : Gtfs_realtime_types.trip_update_stop_time_update)
  encoder
  =
  (match v.Gtfs_realtime_types.stop_sequence with
   | Some x ->
     Pbrt.Encoder.key (1, Pbrt.Varint) encoder;
     Pbrt.Encoder.int32_as_varint x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.stop_id with
   | Some x ->
     Pbrt.Encoder.key (4, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.arrival with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Bytes) encoder;
     Pbrt.Encoder.nested (encode_trip_update_stop_time_event x) encoder
   | None -> ());
  (match v.Gtfs_realtime_types.departure with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Bytes) encoder;
     Pbrt.Encoder.nested (encode_trip_update_stop_time_event x) encoder
   | None -> ());
...
let rec encode_trip_update (v : Gtfs_realtime_types.trip_update) encoder =
  ...
  (match v.Gtfs_realtime_types.vehicle with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Bytes) encoder;
...
let rec encode_vehicle_position_vehicle_stop_status
...
let rec encode_vehicle_position_congestion_level
...
let rec encode_vehicle_position_occupancy_status
...
let rec encode_vehicle_position (v : Gtfs_realtime_types.vehicle_position) encoder =
  (match v.Gtfs_realtime_types.trip with
   | Some x ->
     Pbrt.Encoder.key (1, Pbrt.Bytes) encoder;
  ...
  (match v.Gtfs_realtime_types.vehicle with
   | Some x ->
     ...
     Pbrt.Encoder.nested (encode_vehicle_descriptor x) encoder
   | None -> ());
  (match v.Gtfs_realtime_types.position with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Bytes) encoder;
     Pbrt.Encoder.nested (encode_position x) encoder
   | None -> ());
  (match v.Gtfs_realtime_types.current_stop_sequence with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Varint) encoder;
     Pbrt.Encoder.int32_as_varint x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.stop_id with
   | Some x ->
     ...
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.current_status with
   | Some x ->
     Pbrt.Encoder.key (4, Pbrt.Varint) encoder;
  ...
  (match v.Gtfs_realtime_types.timestamp with
   | Some x ->
     ...
     Pbrt.Encoder.int64_as_varint x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.congestion_level with
   | Some x ->
     Pbrt.Encoder.key (6, Pbrt.Varint) encoder;
     encode_vehicle_position_congestion_level x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.occupancy_status with
   | Some x ->
...
let rec encode_time_range (v : Gtfs_realtime_types.time_range) encoder =
  (match v.Gtfs_realtime_types.start with
   | Some x ->
     Pbrt.Encoder.key (1, Pbrt.Varint) encoder;
     Pbrt.Encoder.int64_as_varint x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.end_ with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Varint) encoder;
     Pbrt.Encoder.int64_as_varint x encoder
   | None -> ());
...
let rec encode_entity_selector (v : Gtfs_realtime_types.entity_selector) encoder =
  (match v.Gtfs_realtime_types.agency_id with
   | Some x ->
     Pbrt.Encoder.key (1, Pbrt.Bytes) encoder;
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.route_id with
   | Some x ->
     ...
     Pbrt.Encoder.string x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.route_type with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Varint) encoder;
     Pbrt.Encoder.int32_as_varint x encoder
   | None -> ());
...
let rec encode_alert (v : Gtfs_realtime_types.alert) encoder =
  ...
  (match v.Gtfs_realtime_types.cause with
   | Some x ->
     Pbrt.Encoder.key (6, Pbrt.Varint) encoder;
     encode_alert_cause x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.effect with
   | Some x ->
     Pbrt.Encoder.key (7, Pbrt.Varint) encoder;
     encode_alert_effect x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.url with
   | Some x ->
     Pbrt.Encoder.key (8, Pbrt.Bytes) encoder;
     Pbrt.Encoder.nested (encode_translated_string x) encoder
   | None -> ());
  (match v.Gtfs_realtime_types.header_text with
   | Some x ->
     Pbrt.Encoder.key (10, Pbrt.Bytes) encoder;
     Pbrt.Encoder.nested (encode_translated_string x) encoder
   | None -> ());
  (match v.Gtfs_realtime_types.description_text with
   | Some x ->
...
let rec encode_feed_entity (v : Gtfs_realtime_types.feed_entity) encoder =
  ...
  (match v.Gtfs_realtime_types.is_deleted with
   | Some x ->
     Pbrt.Encoder.key (2, Pbrt.Varint) encoder;
     Pbrt.Encoder.bool x encoder
   | None -> ());
  (match v.Gtfs_realtime_types.trip_update with
   | Some x ->
     Pbrt.Encoder.key (3, Pbrt.Bytes) encoder;
  ...
  (match v.Gtfs_realtime_types.vehicle with
   ...
   | None -> ());
  (match v.Gtfs_realtime_types.alert with
   | Some x ->
     Pbrt.Encoder.key (5, Pbrt.Bytes) encoder;

...
let rec default_trip_replacement_period 
  ...
  ?replacement_period:((replacement_period:Gtfs_realtime_types.time_range option) = None)
...
let rec default_nyct_feed_header 
  ?nyct_subway_version:((nyct_subway_version:string) = "")
  ?trip_replacement_period:((trip_replacement_period:trip_replacement_period list) = [])
  () : nyct_feed_header  = {
  nyct_subway_version;
  trip_replacement_period;
}

let rec default_nyct_trip_descriptor_direction () = (North:nyct_trip_descriptor_direction)

let rec default_nyct_trip_descriptor 
  ?train_id:((train_id:string option) = None)
  ?is_assigned:((is_assigned:bool option) = None)
  ?direction:((direction:nyct_trip_descriptor_direction option) = None)
...
let rec default_nyct_stop_time_update 
  ?scheduled_track:((scheduled_track:string option) = None)
  ?actual_track:((actual_track:string option) = None)

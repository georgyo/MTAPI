...
let rec default_feed_header_incrementality () : feed_header_incrementality = Full_dataset

let rec default_feed_header
  ?(gtfs_realtime_version : string = "")
  ?(incrementality : feed_header_incrementality option =
    Some (default_feed_header_incrementality ()))
...
let rec default_trip_descriptor_schedule_relationship ()
...
let rec default_trip_descriptor
  ?(trip_id : string option = None)
  ?(route_id : string option = None)
  ?(direction_id : int32 option = None)
  ?(start_time : string option = None)
  ?(start_date : string option = None)
  ?(schedule_relationship : trip_descriptor_schedule_relationship option = None)
  ()
  : trip_descriptor
  =
...
let rec default_vehicle_descriptor
  ?(id : string option = None)
  ?(label : string option = None)
  ?(license_plate : string option = None)
  ()
  : vehicle_descriptor
  =
...
let rec default_trip_update_stop_time_event
  ?(delay : int32 option = None)
  ?(time : int64 option = None)
  ?(uncertainty : int32 option = None)
  ()
...
let rec default_trip_update_stop_time_update_schedule_relationship ()
...
let rec default_trip_update_stop_time_update
  ?(stop_sequence : int32 option = None)
  ?(stop_id : string option = None)
  ?(arrival : trip_update_stop_time_event option = None)
  ?(departure : trip_update_stop_time_event option = None)
  ?(schedule_relationship : trip_update_stop_time_update_schedule_relationship option =
    Some (default_trip_update_stop_time_update_schedule_relationship ()))
  ()
  : trip_update_stop_time_update
  =
  { stop_sequence; stop_id; arrival; departure; schedule_relationship }
;;

let rec default_trip_update
  ?(trip : trip_descriptor = default_trip_descriptor ())
  ?(vehicle : vehicle_descriptor option = None)
  ?(stop_time_update : trip_update_stop_time_update list = [])
  ?(timestamp : int64 option = None)
  ?(delay : int32 option = None)
  ()
  : trip_update
  =
  { trip; vehicle; stop_time_update; timestamp; delay }
;;

let rec default_position
  ?(latitude : float = 0.)
  ?(longitude : float = 0.)
  ?(bearing : float option = None)
  ?(odometer : float option = None)
  ?(speed : float option = None)
  ()
  : position
  =
...
let rec default_vehicle_position_vehicle_stop_status ()
  : vehicle_position_vehicle_stop_status
  =
  Incoming_at
;;

let rec default_vehicle_position_congestion_level () : vehicle_position_congestion_level =
  Unknown_congestion_level
;;

let rec default_vehicle_position_occupancy_status () : vehicle_position_occupancy_status =
  Empty
;;

let rec default_vehicle_position
  ?(trip : trip_descriptor option = None)
  ?(vehicle : vehicle_descriptor option = None)
  ?(position : position option = None)
  ?(current_stop_sequence : int32 option = None)
  ?(stop_id : string option = None)
  ?(current_status : vehicle_position_vehicle_stop_status option =
    Some (default_vehicle_position_vehicle_stop_status ()))
  ?(timestamp : int64 option = None)
  ?(congestion_level : vehicle_position_congestion_level option = None)
  ?(occupancy_status : vehicle_position_occupancy_status option = None)
  ()
  : vehicle_position
  =
  { trip
...
let rec default_time_range ?(start : int64 option = None) ?(end_ : int64 option = None) ()
  : time_range
  =
  { start; end_ }
;;
...
let rec default_entity_selector
  ?(agency_id : string option = None)
  ?(route_id : string option = None)
  ?(route_type : int32 option = None)
  ?(trip : trip_descriptor option = None)
  ?(stop_id : string option = None)
  ()
  : entity_selector
  =
  { agency_id; route_id; route_type; trip; stop_id }
;;

let rec default_alert_cause () : alert_cause = Unknown_cause
let rec default_alert_effect () : alert_effect = No_service
...
let rec default_alert
  ?(active_period : time_range list = [])
  ?(informed_entity : entity_selector list = [])
  ?(cause : alert_cause option = Some (default_alert_cause ()))
  ?(effect : alert_effect option = Some (default_alert_effect ()))
  ?(url : translated_string option = None)
  ?(header_text : translated_string option = None)
  ?(description_text : translated_string option = None)
  ()
  : alert
  =
  { active_period; informed_entity; cause; effect; url; header_text; description_text }
;;

let rec default_feed_entity
  ?(id : string = "")
  ?(is_deleted : bool option = Some false)
  ?(trip_update : trip_update option = None)
  ?(vehicle : vehicle_position option = None)
  ?(alert : alert option = None)
  ()
  : feed_entity
  =
  { id; is_deleted; trip_update; vehicle; alert }
;;

let rec default_feed_message
  ?(header : feed_header = default_feed_header ())
  ?(entity : feed_entity list = [])
  ()
  : feed_message
  =
